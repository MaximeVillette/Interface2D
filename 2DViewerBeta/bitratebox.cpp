...
BitRateBox::BitRateBox(QWidget *parent) :QComboBox(parent),m_customSpeedValidator(new QIntValidator(0, 1000000, this))
{
    fillBitRates();
    connect(this, QOverload<int>::of(&QComboBox::currentIndexChanged),this, &BitRateBox::checkCustomSpeedPolicy);
}
...
{
    if (currentIndex() == (count() - 1)) return currentText().toInt();
...
void BitRateBox::fillBitRates()
{
    ...
    clear();
    for (int rate : rates) addItem(QString::number(rate), rate);

    if (isFlexibleDataRateEnabled())
    {
        for (int rate : dataRates) addItem(QString::number(rate), rate);
    }
    addItem(tr("Manuel"));
