...
QValidator::State HexIntegerValidator::validate(QString &input, int &) const
{
    ...
    if (!ok || value > m_maximum) return Invalid;
    return Acceptable;
}
...
QValidator::State HexStringValidator::validate(QString &input, int &pos) const
{
    ...
    if ((data.size() > maxSize) || (data.size() == maxSize && input.endsWith(space))) return Invalid;
    // check if all input is valid
    const QRegularExpression re(QStringLiteral("^[[:xdigit:]]*$"));
    if (!re.match(data).hasMatch()) return Invalid;
...
SendFrameBox::SendFrameBox(QWidget *parent) :QGroupBox(parent),m_ui(new Ui::MessagesEnvoyes)
{
    m_ui->setupUi(this);
    ...
    auto frameIdTextChanged = [this]()
    {
        ...
        m_ui->B_envoyer->setEnabled(hasFrameId);
        m_ui->B_envoyer->setToolTip(hasFrameId ? QString() : tr("Cannot send because no Frame ID was given."));
    };
    connect(m_ui->frameIdEdit, &QLineEdit::textChanged, frameIdTextChanged);
    frameIdTextChanged();
    connect(m_ui->B_envoyer, &QPushButton::clicked, [this]()
    {
        ...
        if (m_ui->errorFrame->isChecked()) frame.setFrameType(QCanBusFrame::ErrorFrame);
